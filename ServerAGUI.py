...
root.title("Adam's Chatting App.")
...
textScroll= scrolledtext.ScrolledText(root, width=30, height= 25)
...
def display_image():
    # Open the image file
    image_path = r"/home/kali/Desktop/catto.jpeg"
...
# user: Adam
# IP: 127.0.0.2 and port number 2223

ip, port = ("127.0.0.1", 2223)
name = "Maria"
...
def receivefile():
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.bind(('127.0.0.2', 8000))
    fileno = 0
    while True:
        sock.listen(1)
        conn = sock.accept()
        fn = 'RcvFileFromMaria('+str(fileno)+')'
        fileno = fileno+1
        fo = open(fn, "wb")
